...
@section('title', 'Our Projects | Meditrust Nepal')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap gap-3 mb-10" id="project-filters">
            ...
            @foreach($categories as $cat)
                <button type="button"
                        class="project-filter px-5 py-2.5 rounded-full text-sm font-medium transition-all border {{ $cat === 'All' ? 'bg-primary-600 text-white shadow-md border-primary-600' : 'bg-white text-gray-600 hover:bg-primary-50 hover:text-primary-600 border-gray-200 dark:bg-dark-card dark:text-gray-300 dark:border-dark-border' }}"
                        data-cat="{{ $cat }}">
                    {{ $cat }}
                </button>
            @endforeach
            ...
            @foreach($projects as $project)
                ...
                <div class="project-card bg-white dark:bg-dark-card rounded-xl overflow-hidden shadow-sm hover:shadow-md group border border-gray-100 dark:border-dark-border cursor-pointer transition-all"
                    ...
                    <div class="relative h-56 bg-gray-100 dark:bg-gray-800 overflow-hidden">
                        @if($pImg)
                            <img src="{{ $pImg }}" alt="{{ $pTitle }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" loading="lazy" />
                        @else
                            <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-100 to-primary-200 dark:from-primary-900/40 dark:to-primary-800/40">
                                <div class="text-center">
                                    ...
                                    <p class="text-primary-500 text-sm font-medium">{{ $pCat }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($pYear)
                            <span class="absolute top-3 right-3 bg-white/90 text-gray-700 text-xs px-3 py-1 rounded-full font-medium">{{ $pYear }}</span>
                        @endif
                    </div>
                    <div class="p-6">
                        <span class="text-primary-600 text-xs font-medium">{{ $pCat }}</span>
                        <h3 class="font-semibold text-gray-900 dark:text-white mt-1 group-hover:text-primary-600 transition-colors">{{ $pTitle }}</h3>
                        <p class="text-gray-500 dark:text-gray-400 text-sm mt-2 line-clamp-2">{{ $pDesc }}</p>
                    </div>
                </div>
            @endforeach
        </div>
