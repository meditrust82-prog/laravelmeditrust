...
@section('title', 'Contact Meditrust Nepal | Get a Medical Equipment Quote')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
            ...
            <div class="lg:col-span-2 space-y-5">
                @foreach([
                ...
                ] as [$icon,$cls,$label,$value,$href,$link])
                    <div class="bg-white dark:bg-dark-card rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-dark-border flex items-start gap-4">
                        <div class="w-12 h-12 {{ $cls }} rounded-xl flex items-center justify-center text-xl flex-shrink-0">{{ $icon }}</div>
                        <div>
                            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-0.5">{{ $label }}</p>
                            @if($link)
                                <a href="{{ $href }}" class="font-semibold text-gray-900 dark:text-white hover:text-primary-600 transition-colors">{{ $value }}</a>
                            @else
                ...
                {{-- WhatsApp Quick Link --}}
                <a href="https://wa.me/9779818100515?text={{ urlencode('Hello Meditrust Nepal, I would like to request a quote.') }}"
