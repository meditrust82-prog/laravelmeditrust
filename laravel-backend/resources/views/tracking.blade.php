...
@section('title', 'Track Request #' . ($trackingId ?? '') . ' | Meditrust Nepal')
    ...
    @if(isset($trackingId) && $trackingId)
        ...
        <div class="bg-white dark:bg-dark-surface border border-gray-100 dark:border-dark-border rounded-2xl p-6 shadow-sm mb-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <p class="font-bold text-gray-900 dark:text-white">Tracking ID: {{ $trackingId }}</p>
            ...
            @if(isset($order) && $order)
                ...
                @endphp
                <span class="inline-flex px-3 py-1 rounded-full text-xs font-bold {{ $info['color'] }} mb-3">{{ $info['label'] }}</span>
                ...
                @if(!empty($order['items']))
                    ...
                    <ul class="space-y-1 mb-4">
                        @foreach($order['items'] as $item)
                            <li class="text-sm text-gray-600 dark:text-gray-400">• {{ $item['name'] ?? 'Item' }} × {{ $item['quantity'] ?? 1 }}</li>
                        @endforeach
                    </ul>
                @endif
            @else
