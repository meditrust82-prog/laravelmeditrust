...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://127.0.0.1:8001/api/v1' : '/api/v1'),
  withCredentials: true,
});

/* ── Normalize product payload to frontend-friendly shape ── */
...
const normalizeResponse = (data) => {
  ...
  if (Array.isArray(data.products)) {
    return { ...data, products: normalizeResponse(data.products) };
  }
