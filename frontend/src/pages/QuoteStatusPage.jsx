...
export default function QuoteStatusPage() {
  ...
  const handleSearch = async (e) => {
    ...
    setQuotes(null);
    try {
      const isOrderId = /^[0-9]+$/.test(phone.trim()) && phone.trim().length < 8;
      if (isOrderId) {
        const res = await api.get(`/tracking/${phone.trim()}`);
        setQuotes([{
          _id: res.data.orderId,
          productName: `Order #${res.data.orderId} - ${res.data.items.map(i => i.name).join(', ')}`,
          hospitalName: res.data.customer?.name,
          status: res.data.orderStatus,
          createdAt: res.data.lastUpdated,
      ...
      setError('Could not find any quotes or orders for that number. Please check and try again.');
    } finally {
      setLoading(false);
    }
  };
